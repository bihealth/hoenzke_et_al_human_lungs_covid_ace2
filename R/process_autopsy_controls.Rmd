---
title: "process lung autopsy controls"
output: 
  html_document:
    df_print: paged
    code_folding: hide
...
results on lung autopsy tissue with 10X platform. 2 samples (2 donors).  alignment + quantification using `cellranger` against GRCh38 genome. we take all cells with at least 500 genes and mark doublets called by [scrublet](https://github.com/swolock/scrublet)

```{r get_data,fig.width=8,fig.height=4}
samples <- c('BB73-control',
             '17B-control')
...
```{r save_data}
saveRDS(sobj,file='../data/seurat/lung_autopsy_controls.rds')
```
